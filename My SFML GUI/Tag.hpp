...
//tag["name"]["subname"];当"name"键也为Tag类型时可直接使用
...
public:
	...
	//用法:tag.contain("name1", value1, "name2", value2, ...);
	//需要保证参数个数为偶数(因为是键值对)
	//返回:是否包含键值对
...
public:
	Tag() {}
	Tag(std::initializer_list<castable_string> list) {
		for (auto i = list.begin(); i != list.end(); i++) {
